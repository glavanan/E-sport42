...
class MatchViewSet(viewsets.ModelViewSet):
    ...
    def end_match(self, request, pk=None, parent_lookup_tournoi=None, parent_lookup_phase=None):
        ...
        round_max = int(math.ceil(math.log(tournoi(tournoi.nb_teams, 2))))
        if match.phase.name == 'DTree':
            if match.looser_braket == False:
                ...
                next_match_w = Match.objects.filter(match_number=n_winner, level=l_winner, phase=match.phase, looser_bracket=True if match.level==round_max else False)
                next_match_w(team1=match.team1 if match.score_t1 > match.score_t2 else match.team2)
                next_match_l = Match.objects.filter(match_number=n_looser, level=l_looser, phase=match.phase, looser_bracket=True)
                ...
                if match.level == 0:
                    ...
                    l_winner = match.level + 0.5
                next_match_w == Match.objects.filter(match_number=n_winner, level=l_winner, phase=match.phase, looser_bracket=True)
                if match.level % 1 == 0 or (match.match_number % 2 != 0):
